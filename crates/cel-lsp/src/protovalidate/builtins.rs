...
use crate::types::{Arity, CelType, FunctionDef, FunctionKind};

/// Protovalidate-specific extension functions.
pub static PROTOVALIDATE_BUILTINS: &[FunctionDef] = &[
    ...
    FunctionDef {
        ...
        kind: FunctionKind::Method(&[CelType::List]),
        standalone_arity: None,
        method_arity: Some(Arity::Exact(0)),
    },
    ...
    FunctionDef {
        ...
        kind: FunctionKind::Method(&[CelType::Double]),
        standalone_arity: None,
        method_arity: Some(Arity::Exact(0)),
    },
    FunctionDef {
...
/// Pre-computed HashSet for O(1) protovalidate builtin existence check.
static PROTOVALIDATE_SET: LazyLock<HashSet<&'static str>> =
    LazyLock::new(|| PROTOVALIDATE_BUILTINS.iter().map(|f| f.name).collect());

/// Check if a name is a protovalidate extension function.
pub fn is_protovalidate_builtin(name: &str) -> bool {
    PROTOVALIDATE_SET.contains(name)
}
...
pub fn get_protovalidate_builtin(name: &str) -> Option<&'static FunctionDef> {
    PROTOVALIDATE_BUILTINS.iter().find(|f| f.name == name)
}
...
mod tests {
    ...
    fn all_protovalidate_builtins_are_methods() {
        for builtin in PROTOVALIDATE_BUILTINS {
            assert!(
